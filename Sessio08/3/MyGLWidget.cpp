...
void MyGLWidget::paintGL () {
  ...
  if (display_patricio) {
      ...
      //Pintem l'objecte
      glDrawArrays(GL_TRIANGLES, 0, 3 * model_legoman.faces().size());
...
void MyGLWidget::radiEsferaContenidora(){
  ...
  //Els calculem a mà (pel que estem fent no té sentit fer-ho amb una funció)
  xmin = -2.0;
  xmax = 2.0;
  ymin = 0.0;
  ymax = 1.0;
  ...
  //Calculem els parametres d'en Patricio
  calculCapsaContenidoraPatricio();
  calculCapsaContenidoraLegoman();
  //Calculem el radi
  float dx  = xmax - xmin;
  float dy  = ymax - ymin;
  float dz  = zmax - zmin;
...
//Funció que calcula els mínims i els maxims dels patricios
void MyGLWidget::calculCapsaContenidoraPatricio() {
    //Ens recorrem tots els vertex del model en totes les coordenades, buscant els
    //elements mínim i màxim
    xminPatricio = xmaxPatricio = model.vertices()[0];
    yminPatricio = ymaxPatricio = model.vertices()[1];
    zminPatricio = zmaxPatricio = model.vertices()[2];
    for (unsigned int i = 3; i < model.vertices().size(); i += 3)
    {
      if (model.vertices()[i+0] < xminPatricio)
        xminPatricio = model.vertices()[i+0];
      if (model.vertices()[i+0] > xmaxPatricio)
        xmaxPatricio = model.vertices()[i+0];
      if (model.vertices()[i+1] < yminPatricio)
        yminPatricio = model.vertices()[i+1];
      if (model.vertices()[i+1] > ymaxPatricio)
        ymaxPatricio = model.vertices()[i+1];
      if (model.vertices()[i+2] < zminPatricio)
        zminPatricio = model.vertices()[i+2];
      if (model.vertices()[i+2] > zmaxPatricio)
        zmaxPatricio = model.vertices()[i+2];
    }

    //Calculem el centre de la esfera contenidora
    centrePatricio[0] = (xmaxPatricio + xminPatricio) / 2.0;
    centrePatricio[1] = (ymaxPatricio + yminPatricio) / 2.0;
    centrePatricio[2] = (zmaxPatricio + zminPatricio) / 2.0;
}

//Funció que calcula els mínims i els maxims dels patricios
void MyGLWidget::calculCapsaContenidoraLegoman() {
    //Ens recorrem tots els vertex del model en totes les coordenades, buscant els
    //elements mínim i màxim
    xminLegoman = xmaxLegoman = model_legoman.vertices()[0];
    yminLegoman = ymaxLegoman = model_legoman.vertices()[1];
    zminLegoman = zmaxLegoman = model_legoman.vertices()[2];
    for (unsigned int i = 3; i < model_legoman.vertices().size(); i += 3)
    {
      if (model_legoman.vertices()[i+0] < xminLegoman)
        xminLegoman = model_legoman.vertices()[i+0];
      if (model_legoman.vertices()[i+0] > xmaxLegoman)
        xmaxLegoman = model_legoman.vertices()[i+0];
      if (model_legoman.vertices()[i+1] < yminLegoman)
        yminLegoman = model_legoman.vertices()[i+1];
      if (model_legoman.vertices()[i+1] > ymaxLegoman)
        ymaxLegoman = model_legoman.vertices()[i+1];
      if (model_legoman.vertices()[i+2] < zminLegoman)
        zminLegoman = model_legoman.vertices()[i+2];
      if (model_legoman.vertices()[i+2] > zmaxLegoman)
        zmaxLegoman = model_legoman.vertices()[i+2];
    }

    //Calculem el centre de la esfera contenidora
    centreLegoman[0] = (xmaxLegoman + xminLegoman) / 2.0;
    centreLegoman[1] = (ymaxLegoman + yminLegoman) / 2.0;
    centreLegoman[2] = (zmaxLegoman + zminLegoman) / 2.0;
}

//Funció que genera la Model Matrix del primer dels Patricios
void MyGLWidget::modelTransformPatricio1 () {
  //Generem la matriu amb la identitat
  glm::mat4 transform (1.0f);
  float alçada_patricio = (ymaxPatricio - yminPatricio);
  //Apliquem les transformacions que volguem
  transform = glm::translate(transform, glm::vec3(1.0, 0.0, 1.0));
  transform = glm::scale(transform, glm::vec3 (1.0/alçada_patricio, 1.0/alçada_patricio, 1.0/alçada_patricio));
...
//Funció que genera la Model Matrix del segon Patricio
...
void MyGLWidget::modelTransformLegoman1() {
    //Generem la matriu amb la identitat
    glm::mat4 transform (1.0f);
    float alçada_legoman = (ymaxLegoman - yminLegoman);
    //Apliquem les transformacions que volguem
    transform = glm::translate(transform, glm::vec3(1.0, 0.0, 1.0));
    transform = glm::scale(transform, glm::vec3 (1.0/alçada_legoman, 1.0/alçada_legoman, 1.0/alçada_legoman));
    transform = glm::translate(transform, -centreLegoman);
...
//Funció que genera la Model Matrix del terra
...
void MyGLWidget::createBuffersLegoman () {
  //Carreguem les dades del nostre objecte a pintar
  model_legoman.load("../../models/legoman.obj");
  ...
  //A partir del VAO generem el VBO de vèrtex
  glGenBuffers(1, &VBO_vertexs_legoman);
  glBindBuffer(GL_ARRAY_BUFFER, VBO_vertexs_legoman);
  //Omplim el VBO amb les dades
  glBufferData(GL_ARRAY_BUFFER,
               sizeof(GLfloat) * model_legoman.faces().size() * 3 * 3,
               model_legoman.VBO_vertices(), GL_STATIC_DRAW);
  glVertexAttribPointer(vertexLoc, 3, GL_FLOAT, GL_FALSE, 0, 0);
  glEnableVertexAttribArray(vertexLoc);

  //El mateix que abans, pero amb els colors
  glGenBuffers(1, &VBO_color_legoman);
  glBindBuffer(GL_ARRAY_BUFFER, VBO_color_legoman);
  glBufferData(GL_ARRAY_BUFFER,
               sizeof(GLfloat) * model_legoman.faces().size() * 3 * 3,
               model_legoman.VBO_matdiff(), GL_STATIC_DRAW);
  glVertexAttribPointer(colorLoc, 3, GL_FLOAT, GL_FALSE, 0, 0);
  glEnableVertexAttribArray(colorLoc);
...
//Funció que s'activa quan polsem el botó del ratolí
void MyGLWidget::mousePressEvent (QMouseEvent *e){
      makeCurrent();
      //Obtenim la posició als dos eixos a o s'ha fet el click
      xClick = e->x();
      yClick = e->y();

      //Si s'ha apretat el botó, indiquem que s'ha de rotar. Si no esta apretat,
      //No s'ha de fer res
...
//Funció que desactiva la interacció que s'estigues produint, si s'estava
//produint alguna
...
//Funció que enregistra els girs del ratolí i transforma els angles d'Euler en
//consequencia
void MyGLWidget::mouseMoveEvent(QMouseEvent *e) {
      makeCurrent();
      //Distancia que s'ha desplaçat el ratolí en els dos eixos (En valor absolut)
      int dx = abs(e->x() - xClick);
      int dy = abs(e->y() - yClick);

      //Calculem el nou angle "psi" (eix Y)
      if (dx > dy){
        //En base a la direcció del gir augmentem o disminuim psi
        if (e->x() > xClick){
      ...
      //Calculem el nou angle "theta" (eix X)
      else{
        if (e->y() > yClick){
...
void MyGLWidget::changeModel(bool model_changed) {
    makeCurrent();
    display_patricio = model_changed;
    update();
}
